...
import { BsStarFill, BsArrowUpRight, BsArrowDownRight } from 'react-icons/bs'
import moment from 'moment'
import { getCurrency } from '../_utils'
import { Spinner } from 'react-bootstrap'
import { FiAlertTriangle } from 'react-icons/fi'
import { GiPayMoney, GiReceiveMoney } from 'react-icons/gi'

import { AiOutlineAreaChart } from 'react-icons/ai'
import { getIconWithActionAndTooltip, getIconWithTooltip } from '../_components/HoverTooltip'
import CategoryChartModal from '../features/summary/CategoryChartModal'
import { openCategoryChartModal } from '../_components/modal/modal.actions'
import regression from 'regression'
import { hashCode } from '../_utils/hashCode'
...
const SummaryPage = () => {
  ...
  return (
        <Col className="summary-page-container">
            {(isExpenseCategoriesLoading || isIncomeCategoriesLoading)
              ...
              : <>
                    ...
                    <Row className="summary-page-categories-blocks">
                        {expenseCategories
                          ...
                          .map(category => {
                            const avarageLast12Month = countAvarageLast12Month(category?.summaryTransactionMap)
                            const currentMonthAmount = category?.summaryTransactionMap[currentMonth] ?? 0
                            const lastMonthAmount = category?.summaryTransactionMap[previousMonth] ?? 0
                            const suggestedAmount = predictValueForNextMonth(category?.summaryTransactionMap)[1]
                            const plannedAmount = category?.currentVersion ? category?.currentVersion.maximumAmount : 0
                            const isPlannedAmountHasBeenExceed = plannedAmount !== 0 ? currentMonthAmount > category.currentVersion.maximumAmount : false
                            const isSuggestedAmountIsBiggerThanPlanned = plannedAmount !== 0 ? suggestedAmount > plannedAmount : false
